...
#summary of age, BMI, glucose levels, HbA1c levels
summary_stats <- data %>%
  summarise(
    mean_age = mean(age),
...
#calculating sample size of the dataset
...
#stem and leaf plots for the ranges of the continous variables
stem(data$age, scale = 1)
stem(data$bmi, scale = 1)
stem(data$HbA1c_level, scale = 1)
stem(data$blood_glucose_level, scale = 1)




# Ogive for class limits (corrected version)
library(ggplot2)
library(dplyr)
# Function to prepare ogive data
prepare_ogive_data <- function(data, variable, breaks) {
  ...
  freq <- table(cuts)
  # Get upper class limits (using max of each interval)
  upper_limits <- breaks[-1]  # Use the break points (excluding first)
...
# Prepare ogive data for each variable
...
# Prepare frequency polygon data
...
# Function to compute central tendency measures
...
#HYPOTHESIS TEST
#HYPOTHESIS 1:
#Logistic Regression for HbA1c and Diabetes
